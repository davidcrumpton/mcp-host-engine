...
const plugin = {
  ...
  call(params) {
    ...
    let apiUrl, apiToken;
    try {
      apiUrl = host.process.env("HASS_URL") || host.config.options.ApiUrl || "https://homeassistant.local:8123";
      apiToken = host.process.env("HASS_TOKEN") || host.config.options.ApiToken || void 0;
    } catch (err) {
    ...
    switch (params.CommandEvent) {
      ...
      case "set_state": {
        ...
        if (params.state === void 0 || params.state === null) {
      ...
      case "get_calendar_events": {
        ...
        const path = `/api/calendars/${encodeURIComponent(params.calendar_entity_id)}?start=${encodeURIComponent(params.calendar_start)}&end=${encodeURIComponent(params.calendar_end)}`;
        return get(path);
      }
      // POST /api/template
  ...
  _handleResponse(resp, path) {
    var _a;
    const status = (_a = resp.status) != null ? _a : resp.statusCode;
