/**
 * wikipedia_search/plugin.test.ts
 */
...
describe("wikipedia_search plugin", () => {
  it("has the correct metadata", () => {
    expect(plugin.name).toBe("wikipedia_search");
...
const testSearchOpenBSD = async () => { 
  const params = {
    CommandEvent: "CommandEvent.wiki.search",
    Query: "OpenBSD"
  };
  ...
  expect(response.includes("Unix-like")).toBe(true);
  expect(response.includes("BSD")).toBe(true);
  expect(response.includes("ISC")).toBe(true);
};
