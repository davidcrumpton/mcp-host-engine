...
const plugin = {
  name:        "elastic8",
  description: "Fetch data from Elastic8",
  version: "1.1.0",
  commit:      "none",
  Tags:        ["search", "elastic8"],
  annotations: {
    readOnlyHint:    true,
  ...
  call(params: Record<string, unknown>) {
    const baseUrl = host.process.env("ELASTIC_BASE_URL");
    const apiKey  = host.process.env("ELASTIC_API_KEY");

    if (!baseUrl || !apiKey) {
      throw new Error("ELASTIC_BASE_URL and ELASTIC_API_KEY must be set in plugin environment variables");
    }

    host.server.logger(3, "elastic8: fetching from index=" + params.index);
    ...
    try {
      ...
      host.server.logger(3, "elastic8: response status=" + resp.status);
      ...
      host.server.logger(1, "elastic8 error: " + err.message);
